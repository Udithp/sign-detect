...
- **Tkinter (desktop):** Run `run.bat` or `python final_pred.py`
- **Streamlit (web):** Run `run_streamlit.bat` or `streamlit run app_streamlit.py`. Then open the URL shown (e.g. http://localhost:8501), enable **Start camera** in the sidebar, and use the same gestures. Requires `streamlit` and `cvzone` (`pip install streamlit cvzone` if missing). 
...
The SignBridge application is fully configured for production deployment. Since the Flask backend serves the React frontend, it can be deployed as a single, unified web service on platforms like **Render** or **Railway**.

### Deploying on Render (Recommended)
1. Sign up/Log in to [Render](https://render.com/) and create a new **Web Service**.
2. Connect your GitHub repository containing the project.
3. Configure the following settings for the web service:
   - **Environment**: `Python`
   - **Build Command**: `cd web-app && npm install && npm run build && cd .. && pip install -r requirements-prod.txt`
   - **Start Command**: `gunicorn api_server:app --bind 0.0.0.0:$PORT`
4. Deploy the service! 

Render will automatically build the React frontend (placing the output in `web-app/dist`), install the optimized Python backend dependencies (such as CPU-only TensorFlow for smaller footprint), and serve the entire application using Gunicorn.
